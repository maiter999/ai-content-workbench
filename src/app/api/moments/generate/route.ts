...
import { buildMomentsPrompt, MomentsStyle } from '@/lib/prompts/moments'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 验证风格
    const validStyles: MomentsStyle[] = ['吐槽共鸣', '场景种草', '观点日记']
    const style = (validStyles.includes(contentStyle) ? contentStyle : '吐槽共鸣') as MomentsStyle

    // 构建提示词
    const { systemPrompt, userPrompt } = buildMomentsPrompt(style, {
      topic,
      industry,
      requirements,
      materials
