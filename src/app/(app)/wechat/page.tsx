...
const styleOptions = [
  ...
  { value: 'practical', label: '干货清单' },
  { value: 'hot-comment', label: '热点评论' },
...
export default function WechatPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 构建提示词
      let prompt = `请为${industry || '通用'}行业创作一篇适合微信公众号平台的深度文章。\n\n`
      prompt += `主题：${topic}\n`
      ...
      prompt += `\n要求：
1. 标题要有深度和吸引力，控制在25字以内
2. 开头要有引人入胜的引入（150字左右）
3. 正文要有逻辑性，分段清晰，使用小标题
4. 内容要有价值，提供实用信息或独特观点
5. 字数控制在1500-2500字
6. 结尾要有引导关注或互动的内容`
      ...
      const systemPrompt = `你是一个微信公众号内容创作专家，擅长创作专业、有深度的长文。你的文章结构清晰、逻辑严密、有价值输出。你会使用合适的标题层级、列表、加粗等排版元素。`
      ...
      try {
        ...
        // 1. 根据文章内容生成图片提示词
        const imagePromptGenPrompt = `请根据以下公众号文章内容，生成一个适合AI图片生成的提示词（英文）。
要求：
1. 提取文章的核心视觉元素
2. 提示词要详细、具体，适合公众号深度文章风格
3. 图片要专业、有质感
4. 直接输出提示词，不要有任何其他文字
...
${content.substring(0, 500)}`
  ...
  const exportContent = () => {
    ...
    a.href = url
    a.download = `公众号_${topic}_${new Date().getTime()}.txt`
    a.click()
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">微信公众号文章</h1>
        <p className="text-gray-600 mt-1">AI 智能生成适合公众号平台的专业深度文章</p>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder="例如：数字化转型、投资理财、健康管理"
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mb-4">
              ...
              <button
                ...
                className="w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-green-500 hover:text-green-500 transition"
              >
                📷 点击上传参考图片
              </button>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6" style={{minHeight: '500px'}}>
            ...
            {result ? (
              <div className="space-y-4">
                {/* 正文内容 */}
                <div className="bg-gray-50 rounded-lg p-4 whitespace-pre-wrap text-gray-700 leading-relaxed overflow-y-auto" style={{maxHeight: '400px'}}>
                  {result}
                </div>
              ...
              <div className="flex flex-col items-center justify-center text-gray-400" style={{height: '400px'}}>
                ...
                <p>填写左侧信息，点击生成按钮</p>
                <p className="text-sm mt-1">AI 将为你创作优质文章和配图</p>
              </div>
            )}
          </div>
        </div>
